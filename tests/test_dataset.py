...
def _routes(files=None, body=CSV, name="sentiment-mini"):
    ...
    files = files or [{"path": "data.csv", "sha256": _sha(body),
                       "size_bytes": len(body)}]
    calls = []
    ...
    def fake_get(url, **kwargs):
        calls.append(url)
        if "/resolve" in url:
            return _Resp(200, {"canonical": f"zc://alice/{name}@sha256:{DIGEST}",
                               "dataset_id": UUID, "digest": DIGEST,
                               "size_bytes": len(body), "owner": "alice",
                               "name": name})
        if url.endswith(f"/api/datasets/{UUID}"):
            return _Resp(200, {"id": UUID, "name": name, "owner_handle": "alice",
                               "latest_version": {"digest": DIGEST,
                                                  "size_bytes": len(body),
                                                  "files": files}})
...
class TestReferenceRouting:
    def test_a_named_reference_goes_through_resolve(self, monkeypatch, tmp_path):
        ...
        zk.dataset("zc://alice/sentiment-mini", api_url="http://x",
...
class TestLoad:
    def test_csv_becomes_a_list_of_dicts(self, monkeypatch, tmp_path):
        fake_get, _ = _routes()
        monkeypatch.setattr(httpx, "get", fake_get)
        rows = zk.dataset("zc://alice/d", api_url="http://x",
                          cache_dir=tmp_path).load()
        assert rows == [
    ...
    def test_tsv_is_split_on_tabs(self, monkeypatch, tmp_path):
        body = b"id\ttext\n1\thi there\n"
        fake_get, _ = _routes(files=[{"path": "d.tsv", "sha256": _sha(body),
    ...
    def test_jsonl_keeps_native_types(self, monkeypatch, tmp_path):
        ...
        body = b'{"id": 1, "ok": true}\n{"id": 2, "ok": false}\n'
        fake_get, _ = _routes(files=[{"path": "d.jsonl", "sha256": _sha(body),
                                      "size_bytes": len(body)}], body=body)
        monkeypatch.setattr(httpx, "get", fake_get)
        rows = zk.dataset("zc://alice/d", api_url="http://x",
                          cache_dir=tmp_path).load()
    ...
    def test_a_json_array_loads(self, monkeypatch, tmp_path):
        body = b'[{"id": 1}, {"id": 2}]'
        fake_get, _ = _routes(files=[{"path": "d.json", "sha256": _sha(body),
                                      "size_bytes": len(body)}], body=body)
        monkeypatch.setattr(httpx, "get", fake_get)
        rows = zk.dataset("zc://alice/d", api_url="http://x",
                          cache_dir=tmp_path).load()
    ...
    def test_limit_truncates_the_returned_rows(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(httpx, "get", fake_get)
        rows = zk.dataset("zc://alice/d", api_url="http://x",
                          cache_dir=tmp_path).load(limit=1)
        assert len(rows) == 1
    ...
    def test_an_unparseable_format_raises_rather_than_returning_nothing(
        ...
        body = b"PAR1\x00\x00"
        fake_get, _ = _routes(files=[{"path": "d.parquet", "sha256": _sha(body),
                                      "size_bytes": len(body)}], body=body)
        monkeypatch.setattr(httpx, "get", fake_get)
    ...
    def test_a_named_file_can_be_chosen(self, monkeypatch, tmp_path):
        body = CSV
        files = [{"path": "b.csv", "sha256": _sha(body), "size_bytes": len(body)},
                 {"path": "a.csv", "sha256": _sha(body), "size_bytes": len(body)}]
        fake_get, _ = _routes(files=files, body=body)
        monkeypatch.setattr(httpx, "get", fake_get)
        rows = zk.dataset("zc://alice/d", api_url="http://x",
                          cache_dir=tmp_path).load(path="b.csv")
        assert len(rows) == 2
    ...
    def test_an_unknown_file_name_is_an_error(self, monkeypatch, tmp_path):
        ...
        with pytest.raises(FileNotFoundError):
            zk.dataset("zc://alice/d", api_url="http://x",
                       cache_dir=tmp_path).load(path="nope.csv")
...
class TestMetadata:
    def test_exposes_the_resolved_facts(self, monkeypatch, tmp_path):
        fake_get, _ = _routes()
        monkeypatch.setattr(httpx, "get", fake_get)
        d = zk.dataset("zc://alice/sentiment-mini", api_url="http://x",
                       cache_dir=tmp_path)
